...
打开页面时，顶部依次显示加载会话列表、连接并加载资源、恢复历史、初始化扩展；完成后显示原生运行状态。默认命令预览不再阻塞已有会话的连接，模型清单先显示，再同步当前会话的实际模型。

连接失败会持续显示具体原因和“重新连接”按钮。初始连接超过 30 秒会释放请求；已经连上的扩展启动弹窗不受这个超时限制。重新连接只恢复状态和历史，不重发任务。一个会话仍只允许一个页面控制；看到页面占用提示时，应先关闭原控制页面的会话。
