...
- **Dock 功能导航 + 抽屉**：右侧 macOS dock 式图标栏（悬浮放大 + 显示功能名），功能按大分类分组；点击在右侧打开占满阅读器区域的抽屉，关闭即回到默认的阅读器视图
- **图表分析三模块**：🖼 图解析（每张图原图在上、AI 逐图解读直接跟在图下方）、📊 表解析（每张原表在上、逐表解读在下方）、🗺 复现大纲（含 mermaid 实验流水线）；文档骨架按「图N/表N」小节固化，生成后自动挂到对应原图原表下
- **LaTeX·MD 编辑器**：知识库 Markdown 的分屏编辑（源码 + KaTeX 实时预览，与阅读器同一渲染管道）；模型辅助改稿——整篇或选中段落下发指令（润色 / 中译英 / LaTeX 纠错），临时会话返回修改稿，行级 diff 确认后应用，Ctrl+S 保存回知识库
